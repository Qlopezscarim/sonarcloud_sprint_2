...
export const getArduino = (workspaceRef, shouldAlert = true) => {
  ...
  if (JSON.parse(localStorage.getItem('previous_code') != null))
    {
      ...
      previous_code = JSON.parse(localStorage.getItem('previous_code')); //grabbing old code to compare it with the new updated code
    }
  else 
    {
  ...
  code = code.replace("{}","{\n" + custom_code + "\n}") //are appending the initialization code
  ...
  {
    console.log("append here"); //this tells us where to append!!!!!!!!
  ...
  console.log(code.substring(start,end)); //tells us what is new in the code
  localStorage.setItem('previous_code', JSON.stringify(code)); //update the stored "old code" with the "new code"
  return code;
};
